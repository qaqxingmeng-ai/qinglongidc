...
export default function AgentUserDetailPage() {
  ...
  useEffect(() => {
    apiFetch(`/api/agent/users/${userId}`, { method: 'GET' })
      .then(r => r.json())
      .then(json => {
        if (json.success) {
          ...
          setData({
            ...
            identityCode: payload.identityCode ?? null,
  ...
  const saveIdentityCode = async () => {
    ...
    try {
      ...
      setData((prev) => prev ? { ...prev, hasIdentityCode: identityCode.trim() !== '' } : prev);
      setIdentityMessage({ type: 'success', text: '身份码已更新' });
      setTimeout(() => setIdentityMessage(null), 2500);
    } catch (e) {
      setIdentityMessage({ type: 'error', text: e instanceof Error ? e.message : '保存失败' });
    } finally {
  ...
  if (!data) return <div className="text-surface-400 py-20 text-center">用户不存在或无权限查看</div>;
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-6">
        <Link href="/agent/users" className="text-sm text-surface-400 hover:text-surface-500">&larr; 返回</Link>
        <h1 className="section-title">下级用户详情</h1>
      </div>
      ...
      <div className="bg-white rounded-8 border border-surface-100 p-6 mb-6">
          ...
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className={`text-xs px-1.5 py-0.5 rounded ${
                data.role === 'ADMIN' ? 'bg-semantic-danger-light text-red-700' :
        ...
        <div className="mt-6 pt-6 border-t border-surface-100">
          <div className="flex items-center justify-between mb-4">
            ...
            {identityMessage && (
              <span className={`text-xs ${identityMessage.type === 'success' ? 'text-semantic-success' : 'text-semantic-danger'}`}>
          ...
          <div className="flex flex-col md:flex-row gap-3">
            <input
              ...
              placeholder="为下级用户设置或修改身份码"
            />
            <button onClick={saveIdentityCode} disabled={savingIdentity} className="btn-primary btn-sm disabled:opacity-50">
              {savingIdentity ? '保存中...' : '保存身份码'}
            </button>
          </div>
          <p className="text-xs text-surface-400 mt-2">后续邀请码注册时，可按合作商提供的身份码进行绑定。</p>
        </div>
      </div>
            ...
            <div key={s.id} className="flex items-center justify-between px-4 py-3 bg-white rounded-8 border border-surface-100">
              ...
              <div className="flex items-center gap-4">
                <span className={`text-[11px] px-1.5 py-0.5 rounded ${
                  ...
                  s.status === 'EXPIRED' ? 'bg-semantic-danger-light text-red-700' :
          ...
          {data.tickets.length === 0 ? (
            ...
            <div key={t.id} className="flex items-center justify-between px-4 py-3 bg-white rounded-8 border border-surface-100">
              ...
              <div className="flex items-center gap-3">
                ...
                <Link href={`/agent/tickets/${t.id}`} className="text-[11px] text-brand-500 hover:underline ml-2">查看回复</Link>
              </div>
            </div>
          ))}
        </div>
